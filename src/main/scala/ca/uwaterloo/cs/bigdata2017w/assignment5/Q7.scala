...
object Q7 {
  ...
  def main(argv: Array[String]) {
    ...
    if (args.text()) {
      ...
      val l = lineitemRDD
      .filter(lines => lines(10).toString > shipdate)
      .map(lines => {
        (lines(0), ((lines(5).toString.toDouble) * (1 - lines(6).toString.toDouble)))
      })
      .keyBy(data => data._1)

      val c = customerRDD
      .flatMap(lines => List(new Pair(lines(0).toString, lines(1).toString)))

      val cBroadcast = sc.broadcast(c.collectAsMap())

      val o = ordersRDD
      .filter(lines => lines(4).toString < shipdate)
      .filter(lines => !cBroadcast.value.get(lines(1).toString).isEmpty)
      .map(lines => {
        // emit o_orderkey, o_orderdate, o_shippriority, c_name
        (lines(0), lines(4), lines(7), cBroadcast.value.get(lines(1).toString).get)
      })
      .keyBy(data => data._1)
